...
void	escape_char(t_char *curr_char, int *go_line, int *char_to_skip)
{
		...
		*go_line = FALSE;
		if (curr_char->letter == '\'')
			*status = skip_quote(&shell_repr, &curr_char, operand_list);
			...
			escape_char(curr_char, go_line, &to_skip);
		else if (curr_char->letter == '{')
...
int		format_operand_string(t_operand *operand_list, char **operand_str)
{
	...
	ft_dprintf(fd_debug, "here i free\n");
	if (status == MALLOC_ERROR)
...
{
	if (!operand_list)
		ft_dprintf(fd_debug, "NO OPERAND LIST\n");
	else
	{
		while (operand_list)
		{
			ft_dprintf(fd_debug, "{TYPE : %d}-->", operand_list->type);
			operand_list = operand_list->next;
		}
